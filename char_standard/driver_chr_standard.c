...
standard_chr_dev_t  m_standard_chr_dev;      //定义一个标准设备书据结构体


#define CDEV_CNT    1               //设备的个数
#define LED_NAME    "cdev_led"
...
{
    p_filp->private_data = &m_standard_chr_dev ;
    return 0 ;
}
...
{
    standard_chr_dev_t  *p_standard_chr_dev = (standard_chr_dev_t  *)p_filp->private_data;

    printk("\nfile pri data major = %d , minor = %d\n", p_standard_chr_dev->major , p_standard_chr_dev->minor );
    return 0 ;
}
...
static void __exit chr_dev_exit(void)
{
    ...
    cdev_del( &m_standard_chr_dev.cdev );
    unregister_chrdev_region( m_standard_chr_dev.dev_id , CDEV_CNT );

    device_destroy( m_standard_chr_dev.p_dev_class , m_standard_chr_dev.dev_id );
    class_destroy(m_standard_chr_dev.p_dev_class);

    printk("Success: standard chr led exit\n");
}

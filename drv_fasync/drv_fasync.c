 ...
 * wake_up 条件
 */
static bool m_wake_up_condition = false;
...
static ssize_t dev_read( file_t *p_filp , char __user *buff  , size_t cnt , loff_t *offt  )
{
    ssize_t err_code = 0 ;
    ...
    /* */
	wait_event_interruptible(wq, m_wake_up_condition);	//睡眠进程
    m_wake_up_condition = false;
...
//    gpio_irq_dev_t *p_dev = (gpio_irq_dev_t *)p_filp->private_data ;
    ...
    trace_info("event_queue_to_buff:");
...
/* poll接口函数的定义 -- 该函数被应用层的poll接口函数调用 */

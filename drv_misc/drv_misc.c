...
//用户自定义头文件
#define TRACE_MODULE    "drv_misc.c"
...
typedef struct 
{
    ...
    int             minor ;
    nd_t            *node ;
    miscdevice_t    miscdevice;
} dts_chr_dev_t ;

static dts_chr_dev_t  m_dts_chr_dev;            //定义一个设备书书据结构体


#define DRV_NAME_IN_SYS     "drv_misc_led"        //挂载到/dev/目录下的设备节点名称
...
//字符设备操作数据结构体
static file_operations_t m_chr_cdev_fops ={
    .owner = THIS_MODULE ,
    .open  = chr_drv_open,
 ...
 */
    ...
    cdev_init(&m_dts_chr_dev.cdev , &m_chr_cdev_fops );
    ...
    m_dts_chr_dev.miscdevice.name = DRV_NAME_IN_SYS;
    m_dts_chr_dev.miscdevice.fops = &m_chr_cdev_fops;
    m_dts_chr_dev.miscdevice.minor = 144;

    err_code = misc_register(&m_dts_chr_dev.miscdevice);
    if(err_code < 0)
    {
        return err_code;
    }
